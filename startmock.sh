...
if ! [ -e wiremock-standalone.jar ]; then
    ...
    curl https://repo1.maven.org/maven2/com/github/tomakehurst/wiremock-standalone/2.30.1/wiremock-standalone-2.27.1.jar -o wiremock-standalone.jar
    status=$?
    if [ ${status} -ne 0 ]; then
...
java -jar wiremock-standalone.jar --port "$port" --verbose & > $logFile 2>&1 &
echo $! > "$pidFile"
